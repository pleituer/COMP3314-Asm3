...
#### Other features, for example, LBP 
...
We may notice that the accuracy of our LBP model is at around $30\%\sim33\%$, which is much lower than the HOG-based model. Furthermore,even after concatinating the LBP features to our training data and running SVM, we have only obtained an accuracy of $35.8%$.
...
Finally we obtain the confusion matrix on the 80k-20k train test split of the augmented dataset:

![fina_cm](image.png)

We notice the model can distinguish most classes from each other very well, with Cars achieving a $90\%$ accuracy. However, we also notice that it had a very hard time to distinguish between class 3 and 5, which are dogs and cats, which make perfect sense and aligns with the problem of cat and dog classification that a lot of classifiers face.

### Conclusion

Over the 2 weeks, we have obtained pretty impressive result with support vector machines, and with our own innovations, we are able to achieve $80\%$ testing accuracy with only descriptive features that are manually defined and not trained. We also achieved $75\%$ testing accuracy (which can be increased with augmentation) with an incredibly small model, one so small that can be trained in 1 hour. 